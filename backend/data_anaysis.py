...
change_value_dict = {'Collin Creevey':'Colin Creevey', 'Horace Horace Slughorn':'Horace Slughorn', 'Lavander Brown':'Lavender Brown', 'Lily potter':'Lily Potter', 'Pansy parkinson':'Pansy Parkinson', 'Voldemont':'Voldemort', 'Voldermort':'Voldemort', 'Tom Riddle':'Voldemort'}
...
characters_df = characters_df['Name']
characters_df.to_csv("characters.csv")

# 정서 분석
def sentiment_scores(movie_df,n):
    dialog_count = movie_df.groupby('character').count().reset_index().sort_values(['speech'], ascending=False)
    top_ch = dialog_count['character'][:n]
    sentiment_scores_list = []
    
    for ch in (top_ch):
        characters_dialog = list(movie_df[movie_df['character']==ch]['speech'])
...
# emotions dataframe 
emotions_df = sentiment_scores(speech_df,n=30)
emotions_df.to_csv("emotions.csv")

#spells dataframe 수정 
spells_df = pd.read_csv('./backend/data/spells.csv')
change_value_dict = {'Tom Riddle':'Voldemort'}
spells_df = spells_df.replace({'name':change_value_dict})
spells_df = spells_df [['name','series','spell']]
